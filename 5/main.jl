...
function part1(input)
    ...
    for (x1, y1, x2, y2) in input
        if x1 == x2 && y1 != y2
            for coord::Int in range(sort([y1, y2])...)
                counts[(x1, coord)] = get(counts, (x1, coord), 0) + 1
            end
        elseif x1 != x2 && y1 == y2
            for coord::Int in range(sort([x1, x2])...)
...
function part2(input)
    ...
    for (x1, y1, x2, y2) in input
        if x1 - x2 == y1 - y2
            for (coord1::Int, coord2::Int) in zip(range(sort([x1, x2])...), range(sort([y1, y2])...))
                counts[(coord1, coord2)] = get(counts, (coord1, coord2), 0) + 1
            end
        elseif abs(x1 - y2) == abs(x2 - y1)
            for (coord1::Int, coord2::Int) in zip(range(sort([x1, x2])...), reverse(range(sort([y1, y2])...)))
                counts[(coord1, coord2)] = get(counts, (coord1, coord2), 0) + 1
            end
        elseif x1 == x2 && y1 != y2
            for coord::Int in range(sort([y1, y2])...)
